...
pub(crate) fn vk_to_h264_level_idc(level_idc: vk::native::StdVideoH264LevelIdc) -> Result<u8, VulkanDecoderError> {
    match level_idc {
        ...
        _ => Err(VulkanDecoderError::InvalidInputData(format!("unknown StdVideoH264LevelIdc: {level_idc}"))),
    }
}
...
pub(crate) fn h264_level_idc_to_max_dpb_mbs(level_idc: u8) -> Result<u64, VulkanDecoderError> {
    match level_idc {
        ...
        61 => Ok(696_320),
        62 => Ok(696_320),
        _ => Err(VulkanDecoderError::InvalidInputData(format!("unknown h264 level_idc: {level_idc}"))),
    }
}
