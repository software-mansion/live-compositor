...
const scale_down_matrix: mat4x4<f32> = mat4x4(
    vec4(0.45, 0.0, 0.0, 0.0),
    vec4(0.0, 0.45, 0.0, 0.0),
...
// const rotation_matrix_z: mat4x4<f32> = mat4x4(
//     vec4(cos_45, sin_45, 0.0, 0.0),
//     vec4(neg_sin_45, cos_45, 0.0, 0.0),
//     vec4(0.0, 0.0, 1.0, 0.0),
//     vec4(0.0, 0.0, 0.0, 1.0)
// );

// Projection matrix for 3D perspective
const projection_matrix: mat4x4<f32> = mat4x4(
    vec4(1.0, 0.0, 0.0, 0.0),
    vec4(0.0, 1.0, 0.0, 0.0),
    vec4(0.0, 0.0, 1.0, 0.0),
    vec4(0.0, 0.0, 1.0, 0.0)
...
fn in_animation(input: VertexInput, time: f32, plane_id: i32) -> VertexOutput {
    ...
    if (plane_id == 1) {
        x_translation = -0.25;
        y_translation = 0.45;
        z_translation = 0.1 + 0.2;
    }
    // TV
    else if (plane_id == 2) {
        x_translation = 0.0;
        y_translation = 0.0;
        z_translation = 0.2 + 0.2;
    }
    // Bunny
    else if (plane_id == 3) {
        x_translation = 0.25;
    ...
    let start_position = default_bunny_position(input, plane_id);
...
fn default_bunny_position(input: VertexInput, plane_id: i32) -> vec4<f32> {
    if (plane_id == 3) {
...
fn vs_main(input: VertexInput) -> VertexOutput {
    ...
    if (base_params.time < 3.0) {
        output.position =  default_bunny_position(input, base_params.plane_id);
