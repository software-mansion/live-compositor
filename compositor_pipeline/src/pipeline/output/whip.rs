...
fn start_whip_sender_thread(
    ...
    runtime.spawn(async {
        ...
        //     let mut inbound_rtp_packet = vec![0u8; 1500];
        //     while let Ok((n, _)) = video_listener.recv_from(&mut inbound_rtp_packet).await {
        //         if let Err(_) = video_track.write(&inbound_rtp_packet[..n]).await {
        //             error!("Error occurred while writing to video track for session");
        //         }
        //     }
        // });
        ...
        // tokio::spawn(async move {
        //     let mut inbound_rtp_packet = vec![0u8; 1500];
        //     while let Ok((n, _)) = audio_listener.recv_from(&mut inbound_rtp_packet).await {
        //         if let Err(_) = audio_track.write(&inbound_rtp_packet[..n]).await {
        //             error!("Error occurred while writing to video track for session");
        //         }
        //     }
        // });
