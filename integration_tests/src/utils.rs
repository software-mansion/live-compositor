...
pub fn init_example(client_code: fn() -> Result<()>) {
    thread::spawn(move || {
        ffmpeg_next::format::network::init();

        download_all_assets().unwrap();

        if let Err(err) = wait_until_server_alive(Duration::from_secs(10)) {
            error!("{err}");
            process::exit(1);
        }
...
fn wait_until_server_alive(timeout: Duration) -> Result<()> {
    error!("starting server wait");
    ...
    loop {
        match reqwest::blocking::get(server_status_url) {
            Ok(_) => break,
            Err(_) => info!("Waiting for server to start"),
        };
        if Instant::now() - wait_start_time > timeout {
...
fn download_all_assets() -> Result<()> {
    ...
    let mut any_asset_downloaded = false;
    for asset in assets {
        match download_asset(&asset) {
            Ok(()) => any_asset_downloaded = true,
            Err(_) => info!("Error while downloading asset: {:?}", asset),
        };
    }
