...
pub fn start_ffmpeg_receive(video_port: Option<u16>, audio_port: Option<u16>) -> Result<()> {
    let output_sdp_path = match video_port {
        Some(video_port) => match audio_port {
            Some(audio_port) => write_video_audio_example_sdp_file(video_port, audio_port),
            None => write_video_example_sdp_file(video_port),
        },
        None => match audio_port {
            Some(audio_port) => write_audio_example_sdp_file(audio_port),
            None => {
                return Err(anyhow!(
